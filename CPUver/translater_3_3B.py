...
# 日誌記錄器 / Logger
logger = logging.getLogger("translater_3_3B")
    ...
    def __init__(self, cfg: NLLBConfig):
        ...
        try:
            ...
            if lang_codes and cfg.tgt_language in lang_codes:
                ...
                # 對於 NLLB，使用 convert_tokens_to_ids(cfg.tgt_language) 而非 >> <<
                token = cfg.tgt_language
                self.bos_token_id = self.tokenizer.convert_tokens_to_ids(token)
...
def main():
    parser = argparse.ArgumentParser(description="NLLB-200 3.3B 翻譯 (CPU，config 化)")
